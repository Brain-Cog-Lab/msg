...
class aaNet(nn.Module):
    def __init__(self, net, T=4, att=False, neuron=None):
...
def parse_args():
    ...
    parser.add_argument('--saved_dir', type=str, default='/data/ly/msg_aaai')
    parser.add_argument('--saved_csv', type=str, default='./results_aaai.csv')
    ...
    # surrogate
    parser.add_argument('--main_func', type=str, default='PiecewiseLinearGrad', choices=['PiecewiseLinearGrad', 'AtanGrad', 'SigmoidGrad', 'RectangularGrad'])
    parser.add_argument('--sub_func', type=str, default='ZeroGrad', choices=['ZeroGrad', 'DeltaGrad', 'StepGrad'])
    parser.add_argument('--alpha', type=float, default=2.0)
    parser.add_argument('--alpha_max', type=float, default=2.0)
    parser.add_argument('--prob', type=float, default=0.5)
    parser.add_argument('--prob_max', type=float, default=1.0)
...
def change_sg(net, epoch, args):
    if epoch > args.start_epoch:
        alpha = (args.alpha_max - args.alpha) / (args.epochs - 1 - args.start_epoch) * (epoch - args.start_epoch) + args.alpha
        sub_prob = (args.prob_max - args.prob) / (args.epochs - 1 - args.start_epoch) * (epoch - args.start_epoch) + args.prob
    else:
        alpha = args.alpha
        sub_prob = args.prob

    surrofunction = MixedSG(main_func=args.main_func,
                            sub_func=args.sub_func,
                            alpha=alpha,
                            sub_prob=sub_prob,
                            )

    for ind, layer in enumerate(net.net.modules()):
        if hasattr(layer, 'reset') and ind != 0:
            layer.act_func = deepcopy(surrofunction)
...
def train_net(net, train_iter, test_iter, optimizer, scheduler, criterion, device, args=None):
    ...
    for epoch in range(args.epochs):
        ...
        change_sg(net, epoch, args)

        # print(f"epoch{epoch}", net.net.conv[2].act_func.alpha, net.net.conv[2].act_func.sub_prob)
        ...
        if args.save_log:
            ...
            result2csv(os.path.join(args.saved_dir, 'result_aaai.csv'), dic)
...
if __name__ == "__main__":
    ...
    wandb.init(project='MSG_aaai', resume="allow")

...
def parse_args():
    ...
    parser.add_argument('--alpha_max', type=float, default=50.)
...
def change_alpha(net, alpha=2):
    for ind, layer in enumerate(net.net.modules()):
        if hasattr(layer, 'reset') and ind != 0:
            layer.act_func.alpha.data = torch.tensor(alpha, dtype=torch.float)
...
def train_net(net, train_iter, test_iter, optimizer, scheduler, criterion, device, args=None):
    ...
    for epoch in range(args.epochs):
        ...
        change_alpha(net, epoch * (args.alpha_max - 2) / args.epochs + 2)
